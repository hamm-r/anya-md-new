...
const formatDuration = ms => {
  ...
  if (days) arr.push(`${days} Hari`)
  if (hours) arr.push(`${hours} Jam`)
  if (mins) arr.push(`${mins} Menit`)

  return arr.join(', ') || 'Baru jadian 🥰'
}
...
let handler = async (
  ...
  // ================= TEMBAK =================
  if (/^(tembak|jadian)$/i.test(command)) {
...
${usedPrefix}tembak @tag`
    ...
    user = await fixJid(conn, user)

    if (sameUser(user, m.sender))
      return m.reply('Tidak bisa diri sendiri 🗿')

    if (sameUser(user, conn.user?.jid))
      return m.reply('Tidak bisa bot 😹')
    ...
    if (sender.pasangan) {
      return sendMention(
        conn,
        m.chat,
        `💔 Kamu sudah punya pasangan ${toM(sender.pasangan)}`,
        [sender.pasangan],
        m
      )
    }
    ...
    if (target.pasangan) {
      return sendMention(
        conn,
        m.chat,
        `💔 Dia sudah punya pasangan ${toM(user)}`,
        [user],
        m
      )
    }
    ...
    if (target.pending === m.sender) {
      ...
      sender.jadian = Date.now()
      target.jadian = Date.now()
...
${toM(m.sender)} menembak ${toM(user)} 💘

⚠️ Hanya ${toM(user)} yang bisa menjawab!
...
👉 *${usedPrefix}tolak*`,
  ...
  if (/^terima$/i.test(command)) {
    ...
    if (!sender?.dari) {
      return m.reply(
        'Tidak ada yang nembak kamu 🗿'
      )
    }

    let penembak =
      await fixJid(conn, sender.dari)

    let target = db[penembak]

    sender.pasangan = penembak
    target.pasangan = m.sender
...
Semoga langgeng 😆✨`,
  ...
  if (/^pasanganku$/i.test(command)) {
    ...
    let pasangan =
      await fixJid(conn, sender.pasangan)

    let lama = sender.jadian
      ? formatDuration(
          Date.now() - sender.jadian
        )
      : 'Tidak diketahui'
...
Semoga langgeng 😆✨`,
  ...
  if (/^putus$/i.test(command)) {
    ...
    if (db[pasangan]) {
      db[pasangan].pasangan = ''
      db[pasangan].jadian = 0
    }

    return sendMention(
      conn,
      m.chat,
`💔 Putus dengan ${toM(pasangan)}
...
handler.command =
  /^(tembak|jadian|terima|tolak|pasanganku|putus)$/i
